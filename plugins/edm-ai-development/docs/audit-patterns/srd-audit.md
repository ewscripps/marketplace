...
**Fix:** Enumerate migration filenames in §6, not ranges; companion docs link to SRD as canonical.
...
**Fix:** Explicit pre-requisite block in §6: "Before R31 implementation, migration 037 must drop `people_name_key` and add `uq_people_name_station`."
...
- [ ] **Competing requirements scan:** Search for `must` + `must not` pairs that contradict. For each, add a §7 Design Decision explaining the chosen trade-off.
...
- [ ] **Migration envelope:** For any schema changes, §6 enumerates the exact migration filename (not a range). Include a "Prerequisites" row if a prior migration must run first.
- [ ] **API error codes complete:** Grep for `[1-5]\d\d` — every HTTP status code found in prose must appear in the §8 status-code table.
- [ ] **Secret/security anchor:** For every secret/token/credential, verify it appears in §7 Risk and has a documented mitigation. Grep `password|secret|key|token|credential|jwt|api_key`.
...
- Contradictions are **explicitly called out in §7 Design Decisions** with a reasoned choice and mitigations listed.
