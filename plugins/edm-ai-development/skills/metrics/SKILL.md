...
Reports both **time** (wall-clock duration per phase + gate review time) and **cost** (Claude API USD spent, computed from session token usage x current Anthropic pricing) alongside a **human baseline** (estimated developer hours x `${user_config.human_hourly_rate_usd}`) for each phase.
...
- **Human cost** is computed from the Phase Timing Guidelines (median expected hours per phase x `${user_config.human_hourly_rate_usd}`).
- **Savings ratio** = `human_cost / claude_cost`. A 50x ratio means Claude cost 1/50th of what a human team would have. Use it to make the case for AI-assisted development to stakeholders.
