...
> **SELF-INTERROGATION — REQUIRED before finalizing the fix plan.** Answer these three questions explicitly, and route each answer into an artifact (an answer that lives only in your reasoning is wasted):
>
...
>
> 3. **Does this fix extend anything deprecated?** Check the affected areas against the declared deprecated inventory built in B3. If the fix adds to a deprecated surface — a new file in a deprecated subtree, a new member on a deprecated type, a new counterpart to a deprecated file family — say so explicitly and either justify it or rework that step. Deprecated code sits beside its supported counterpart, so fixing the neighbour and mirroring it is the usual way this happens by accident. Route: record the decision under the plan's risks/open items so it is visible at B6. If no inventory is declared, answer "none declared" in one line.
>
