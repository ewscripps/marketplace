...
> **SELF-INTERROGATION — REQUIRED before finalizing the breakdown.** Answer these three questions explicitly, and route each answer into an artifact:
>
...
>
> 3. **Does this breakdown extend anything deprecated?** Resolve the repository's declared deprecated inventory per `deprecated-scope-protocol.md` §1 and check it against the children's affected areas. A child scoped to build on a deprecated surface will not surface the problem until implementation, when it is far more expensive to unwind. Say so explicitly and rescope the child. Route: record it in the affected child's Risks/Dependencies entry. If no inventory is declared, answer "none declared" in one line.
>
