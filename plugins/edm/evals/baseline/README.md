...
AC8; `run-eval.sh`'s `ANTHROPIC_API_KEY` env-var doc row and its "Two sanctioned auth paths"
comment plus the auth gate beneath it -- cited by name, not line range: two successive versions
of a line-range citation here both went stale, the exact drift class G39/CA-315 named, and
CA-464 retired the numeric form) -- an exported `ANTHROPIC_API_KEY`, or a `claude` CLI
