...
**Ordering, stated explicitly (CA-034): no readiness score exists at this point on any run that reaches this step.** Its only producer is `skills/plan/SKILL.md`'s optional Step 6, which runs inside Phase 1 -- dispatched later, at Step 2 -- and this step is skipped on resume, the one run in which a prior `planning.md` could already carry a score. The two cases are disjoint by construction, so the coupling below is a documented input this step does not acquire, never a read it is failing to perform: it acquires nothing, runs nothing, and waits for nothing. When a score IS available (a caller that ran the scorecard first, or a later flow that reorders the two), this step may consult it only as an additional input to the **design-ambiguity** signal above -- read from the `Overall score:` line of `planning.md`'s optional `## Repository Readiness` section, the only artifact that carries it -- never as a fourth signal, and never as an input to the files-touched or new-dependency-or-contract signals. The security-trigger tie-breaker's `standard` floor above always wins over any score-driven adjustment; the score never lowers or overrides it. With no score available -- which is every run today -- this step still produces a recommendation from the three signals alone and never blocks on, or waits for, the scorecard.
...
5. Mode-family fields and each mode's full behavior are `CLAUDE.md Sec."EDM mode matrix (EDMV3-T38)"` --
