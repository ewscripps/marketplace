...
When a repository readiness score is available (`skills/plan/SKILL.md`'s optional Phase 1 step), this step may consult it only as an additional input to the **design-ambiguity** signal above -- never as a fourth signal, and never as an input to the files-touched or new-dependency-or-contract signals. The security-trigger tie-breaker's `standard` floor above always wins over any score-driven adjustment; the score never lowers or overrides it. When no score is available, this step still produces a recommendation from the three signals alone and never blocks on, or waits for, the scorecard.
