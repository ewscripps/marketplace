...
| `1` | Setup error (malformed rule file, unknown operator, out-of-event field, an absent or unparseable `eval` payload on stdin, or `jq` missing) -- **never blocking** |
...
Exit 1 never escalates to a block in either consumer, and both halves of that claim are now proven
through the real binaries rather than a re-typed translation harness (CA-043): a matched `warn`
rule allows with empty stdout and its line on stderr, and a malformed rule file allows with
hookify's setup line on stderr, through `edm-gateguard` and `edm-stop-gate` alike. **The
warn-line-to-stderr half of this contract was untrue through `edm-stop-gate` until CA-008**: it
captured hookify with `2>&1` and printed the capture only on the block path, discarding every warn
line and every setup error. `EDMV4-T45` wires both call sites: `file`
...
`bash` events -- the one surface that could not avoid overlapping the commit-time artifact lint --
ship via `bin/edm-bash-gate` under the `PreToolUse Bash` block, only because Spike A
(decisions.md D25) recorded a positive multi-hook-per-event result. Since CA-007 the artifact lint
sits in that SAME `Bash` block as a second, `if`-gated entry rather than in a separate block of its
own -- D25 AC5's homogeneous command-plus-command result is what makes the two co-registered
entries safe, and the runtime re-verification recorded in the `Bash` rows above confirms neither
suppresses the other. Per AD4, hookify registers no
...
- **This plugin's own source tree is scanned by no invocation the hook makes.** The
  commit-time artifact-lint hook runs prefix mode, which never
  ...
  automatic is checking it. (**Reach is the whole explanation, and it always was.** CA-007 found
  that the hook's matcher never fired either, and it would be easy to read the surviving em dashes
  as evidence of that instead -- they are not. Prefix mode never reaches those directories, and
  `collect_md_files` filters to `*.md` so `bin/` helpers are never collected in ANY mode. Both gaps
  below are independent of whether the hook fires at all.)
...
| `edm-sync-canonical-sections` | Regenerate `docs/canonical-sections.md` from the canonical sections of this file that the generator's own generation block names (byte-identical, one-directional); `--check` exits 1 on drift. Run `edm-sync-canonical-sections --help` for the live inventory -- it derives the set and the count at runtime from that block. This row deliberately restates neither, because a restated count is exactly what went stale here (CA-022, and CA-021 before it). See the note below the Mermaid section for why this file exists (EDMV3-T41). |
...
| `edm-bash-gate`       | `PreToolUse` `Bash`-matcher hookify consumer (`EDMV4-T45`), registered only because Spike A (decisions.md D25) confirmed co-registered `PreToolUse` hooks run without suppressing one another -- it is entry 1 of the `Bash` block, sharing that array with the `if`-gated commit-time artifact lint since CA-007. See "Hooks behavior" below for its full exit-code contract and for CA-039's payload-shape diagnostic. |
...
|---|---|---|---|---|
| **Commit-path** | `edm-lint-artifacts <PREFIX>`, from the `if`-gated commit-time entry of the `PreToolUse Bash` block | **3,000 ms** p95 | one initiative directory of 30 `.md` files / 9,990 lines | Every `git commit` that stages anything under the hook's derived `srd_root` scope (CA-023). A human is waiting on this one, so it is the budget that must stay small. **This budget was never actually spent before CA-007** -- the hook's matcher was the literal `git commit`, which matches no tool name, so the scan never ran on any commit. Treat any pre-CA-007 "the commit hook feels fast" impression as unmeasured |
...
| `code_audit_p2_debt_accepted` / `code_audit_p2_debt_count` / `code_audit_p2_debt_round` / `code_audit_p2_debt_accepted_at` / `code_audit_p2_debt_accepted_by` | boolean / number / number / string (ISO-8601 UTC) / string | none seeded -- all five written together by `approve-gate <PREFIX> code-audit --accept-p2-debt` (EDMV3-T68, D57/D58) | The sanctioned P2-debt acceptance record: `code_audit_p2_debt_count` open P2s carried forward from round `code_audit_p2_debt_round`, accepted by `code_audit_p2_debt_accepted_by` at `code_audit_p2_debt_accepted_at`. **All five names are spelled out because the family is NOT uniformly suffixed** -- the boolean, `_accepted_at` and `_accepted_by` carry `accepted`, while the count and round do NOT (`code_audit_p2_debt_count`, not `..._accepted_count`). This row previously used a `` `_count` / `_round` `` shorthand that a reader naturally expands the wrong way; querying the expanded names returns null, which reads as "the writer did not record it" when in fact the values are there under the shorter names. Found at EDMV4's own convergence gate. The ledger itself is left unchanged (accepted P2s still show open); HANDOFF's code-audit gate row renders all four metadata values (CA-429), and `edm-state archive` compares `_round` against the current `audit_rounds.code.count` to refuse on stale debt | Absent means no debt was ever accepted; every reader uses jq `//` defaults and nothing fails on absence (C-4) |
...
| `partial_verdict_map.<ticket>.closing_verdict` | string enum: `PASS` \| `FAIL` | absent while the entry is open | The closing verdict written by `edm-state record-partial-verdict <PREFIX> <ticket> close <PASS\|FAIL> <ref>`, driven by `/edm:verify-runtime`. There is no third value -- no `BLOCKED`, `WAIVED` or `N/A-runtime` (D15). `archive` hard-blocks on any entry that is unclosed or FAIL-closed. An entry may be closed once, the sole exception being re-closure of a FAIL after remediation. **CA-092 -- the write is locked, and the wait for that lock now backs off and jitters.** The whole read-modify-write already ran under one `with_state_lock` acquisition (CA-160), so this was contention, not a race -- but the spin slept a flat 0.1s, so at 6-10 way Phase 6 contention every waiter woke on the same tick and one unlucky auditor could be starved right through its budget. It then died on timeout, its PARTIAL never reached `partial_verdict_map`, and `archive` stopped blocking on a ticket nobody had closed -- the `OPEN_PARTIALS` guarantee silently weakened by a lock queue rather than by any decision. The spin is now capped-exponential with uniform jitter (`_lock_backoff_seconds`), bounded by a wall-clock `EDM_STATE_LOCK_WAIT_S` deadline rather than by a try count that stopped being a proxy for elapsed time; the first two retries still sleep exactly 0.1s, so lightly-contended runs are unchanged. A final failure still `die`s non-zero and loudly -- a dropped verdict is never silent | Absent means still open; the blocking `OPEN_PARTIALS` anomaly names the ticket. The entire pre-closure entry is preserved under `prior` rather than overwritten, and a re-closure appends to `closure_history` so the FAIL record is never lost |
...
- `EDM_GATEGUARD_EXEMPT_GLOBS` -- comma-separated glob list tested against the target path;
  ...
  `node_modules`, `.git`). Unset uses the shipped default list, never an empty list. **CA-073 --
  the list is split with pathname expansion DISABLED (`set -f`), and the entries are patterns, not
  filenames.** The split is an unquoted array assignment, which applies both word splitting (wanted,
  on `IFS=,`) and pathname expansion (never wanted): with globstar off, `**/SRD/**` behaves as
  `*/SRD/*`, so in any repository where a matching path existed relative to the HOOK's cwd the entry
  expanded into that one literal filename and stopped being a pattern. Every exemption in the list
  then silently stopped working and the gate denied edits to exactly the trees it was configured to
  leave alone. The previous state of `-f` is restored rather than assumed off.
- `EDM_GATEGUARD_STATE_DIR` -- overrides where session state (the checked-file and the denial
  ...
  subdirectory. Unset uses that resolved default. A trailing slash is stripped. The override is
  total: with it set, `<override>/<project-key>.checked` and `<override>/<project-key>.denials`
  are the ONLY places session state lands, and nothing is written under the default `run/`
  directory (the marker itself is unaffected -- it lives on its own data-directory chain).
  CA-044: until this batch the only test that set this variable also set `EDM_GATEGUARD=0`, so the
...
- `EDM_GATEGUARD_MAX_DENIALS` -- caps full denials per session; defaults to `3`. Past the budget
  ...
  **CA-084 -- "per session" was untrue until this fix.** The budget was keyed per PROJECT, so the
  ...
  refusing loudly (a setup slip in an edit-time gate should degrade to the safe default, not block
  every edit) -- the same "a setup condition never blocks" convention this file applies to
  `edm-hookify` and `edm-lint-staged-artifacts`. Before that validation the value reached a bash
