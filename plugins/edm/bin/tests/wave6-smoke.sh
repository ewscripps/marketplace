...
bad_mode_ec=$?
set -e
check_fails "unknown mode fails loudly naming the value" "$bad_mode_ec" "unknown mode 'bogus-mode'" "$bad_mode_out"
...
bad_lc_ec=$?
set -e
check_fails "unknown lifecycle_mode fails loudly naming the value" "$bad_lc_ec" "unknown lifecycle_mode 'bogus-lc'" "$bad_lc_out"
check "unknown lifecycle_mode error lists the legal enum" "standard|partial|fast-track|fix-pack" "$bad_lc_out"

set +e
"$EDM_STATE" set-mode T7SEEDSTD mode bogus-mode >/dev/null 2>&1
bad_setmode_ec=$?
set -e
before_setmode="$(state_snapshot "$STATE_T7SEEDSTD")"
[[ $bad_setmode_ec -ne 0 ]] && pass "set-mode rejects unknown mode value (CLI path)" \
  || fail "set-mode accepted an unknown mode value"
check_state_unchanged "set-mode with unknown mode does not mutate state" "$STATE_T7SEEDSTD" "$before_setmode"
