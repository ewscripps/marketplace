...
- **Hook-spawned (per-implementer) shard**: `<initiative-dir>/qc/qc-shard-impl-w{WW}-{NN}.md`, where `{WW}` is the **wave number** the implementer you are auditing ran in and `{NN}` is the lowest ticket number in your assigned range, both 1-based and zero-padded (e.g. wave 2, tickets T07-T09 -> `qc-shard-impl-w02-07.md`). The wave component is required (CA-010), for the same reason CA-515 required it on the threshold-shard name below: a Step 5 remediation loop re-runs an implementer over the SAME ticket range, so a wave-less name is reused and the later run silently overwrites the earlier wave's shard, discarding its PASS and FAIL verdicts. If you were not told your wave number and cannot determine it, use the lowest `{WW}` for which the file does not already exist -- that preserves the no-overwrite guarantee without inventing a wave you cannot verify.
