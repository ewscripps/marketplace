...
| 1.4.0   | 2026-08-21 | orchestrator      | CR8 (D57/D58, code-audit round-9/10 remediation): EDMV3-90 amended to carve out the single sanctioned `--accept-p2-debt` flag on `approve-gate code-audit`, closing the CA-509 defect where the ticket pack (`tickets/README.md`, `tickets/epics/04-structured-findings.md`) asserted this amendment while `srd.md` still forbade the flag it authorizes. AC1 gains a named carve-out limited to `approve-gate code-audit --accept-p2-debt`; every other command (`phase-complete`, `archive`, `audit-converged`, and `approve-gate` for any gate other than `code-audit`) is unaffected and the flag remains P0/P1-non-waivable by `cmd_approve_gate`'s own hard refusal. AC4 gains a matching carve-out: the recorded `code_audit_p2_debt_accepted` state fields are the one sanctioned recorded-exemption category, distinct from a per-AC `BLOCKED`/`WAIVED` verdict (D15 continues to forbid that). No requirement added or removed; totals stay 120 at 85/25/5/5. |
...
- **Acceptance Criteria**:
    - [ ] No `--force`, `--accept-partials`, `--skip-checks`, `--yes`, or equivalent bypass flag exists on
      `phase-complete`, `archive`, `approve-gate`, or `audit-converged` -- **with exactly one named carve-out (D57/D58,
      CR8)**: `approve-gate code-audit --accept-p2-debt`. The carve-out is scoped to that single subcommand pairing; it
      does not extend to `approve-gate` for any other gate, nor to `phase-complete`, `archive`, or `audit-converged`.
      `--accept-p2-debt` cannot waive an open P0 or P1 (`cmd_approve_gate` hard-refuses regardless of the flag) --
      it waives only open P2 severity, and only at the convergence gate, with the acceptance recorded rather than
      silent (`code_audit_p2_debt_accepted`/`_count`/`_round`/`_accepted_at`/`_accepted_by`, all five written together).
    - [ ] `grep -rn -- '--force\|--accept-partials' plugins/edm/bin plugins/edm/skills plugins/edm/agents` returns zero
    ...
    - [ ] **There is no BLOCKED or equivalent third verdict for an unverifiable acceptance criterion** (D15, EDMV3-117).
      ...
      This is unchanged by CR8's carve-out below: that carve-out operates at the *round* (convergence-gate) granularity,
      accepting a known, named set of open P2 findings as documented debt, never at AC granularity, and never for a
      P0/P1 finding or an unverifiable AC.
    - [ ] **Named exception (D57/D58, CR8):** `approve-gate code-audit --accept-p2-debt` is the one sanctioned
      recorded-exemption category this requirement admits. It differs from the third-verdict category this AC
      otherwise forbids in three ways: it is a round-level convergence decision, not a per-AC verdict; it is P2-only
      and hard-refuses on any open P0/P1; and the acceptance is recorded in state (`code_audit_p2_debt_accepted` and
      siblings) rather than silently passed, so the ledger continues to show the accepted findings as open. No other
      recorded-exemption category exists or is sanctioned by this amendment.
