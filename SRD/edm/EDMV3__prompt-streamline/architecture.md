...
| E1 | git / GitLab | Local `git` CLI; CI over GitLab runners | Developer's existing git credentials; CI job tokens | `git_aware_mv` falls back to `mv`; `edm-state git-lock-check` (`cmd_git_lock_check` in `bin/edm-state`) detects and removes a stale `index.lock`, but it is OPERATOR-INVOKED, not automatic -- nothing in `skills/`, `agents/`, or `hooks/hooks.json` calls it (G87, confidence LOW) |
