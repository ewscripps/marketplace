...
Companion to `srd.md` v1.2.0 Section 4. The SRD states the six decisions AD-DS1 through AD-DS6 in
...
| BashGate | `plugins/edm/bin/edm-bash-gate` | `PreToolUse` hook for Bash -- projects the command out of the payload and delegates to hookify's `bash` event | in: hook JSON on stdin. out: stderr + exit 2 on block (`:135-138`), exit 0 otherwise (`:139`) | `_edm-cli-lib.sh` (**unguarded**, `:69`), `edm-hookify eval bash` (`:131`), `jq` (`:127`) | EDMDS-01 AC3, EDMDS-02 AC2/AC6, EDMDS-05 AC1-AC4, EDMDS-14 AC6, EDMDS-16 AC1/AC7 |
| StopGate | `plugins/edm/bin/edm-stop-gate` | `Stop` hook -- blocks session stop on a blocking `edm-state validate` anomaly or a matched `stop`-event rule, via the one labelled emitter | in: Stop payload on stdin, `stop_hook_active`. out: `stop_gate_emit_blocking <label> <text>` to stderr (`:120-124`), exit 2. Every internal error exits 0 (`soft_exit`, `:107-111`) | `_edm-cli-lib.sh` (**unguarded**, `:65`), `edm-state validate`, `edm-hookify eval stop` | EDMDS-02 AC2/AC6/AC9 (**changed** -- v1.1.0's "confirm only, no change" was true of the file and false of the behaviour: the untrusted half moves to stdout and AC9 verifies the `Stop` stdout claim), EDMDS-04 AC3, EDMDS-12 AC2/AC3, EDMDS-16 AC1/AC7 |
...
| Wave-8 suite | `plugins/edm/bin/tests/wave8-smoke.sh` | Hook-consumer, sanitizer-ordering and data-directory-ownership assertions | `bash wave8-smoke.sh`, `Results:` + exit | all four consumers, `_edm-datadir-lib.sh` | EDMDS-02 AC4/AC7/AC8/AC9, EDMDS-05 AC2-AC4, EDMDS-11 AC8, EDMDS-14 AC2-AC5/AC7, EDMDS-16 AC3/AC5/AC6/AC7, EDMDS-19 AC8/AC9 |
...
| GateGuard, `exit-code`, exits 2 | `bin/edm-gateguard:236-240` | **stderr -- MEASURED** (D52: the nested session quoted the stderr marker back verbatim as the refusal it received) | **stdout -- MEASURED free** (D52: it reached the model only inside the echoed command string, not as captured output) | the author's `message`, unlabelled, indistinguishable from EDM's own prose | AC6 -- Separate: EDM-authored reason to stderr, author's `message` to stdout, sanitized, behind an EDM-authored prefix naming rule id and file |
| BashGate, exits 2 | `bin/edm-bash-gate:136-137` | **stderr -- MEASURED**. D52's spike registered its hook on the `Bash` `PreToolUse` matcher, so this is the row that was measured directly and the GateGuard row above is its sibling on the same event | **stdout -- MEASURED free** | same | AC6, same Separate shape. EDMDS-14 AC6 gives this script the sanitizer it lacks |
| StopGate, exits 2 | `bin/edm-stop-gate:216` and `:244` | stderr -- **NOT measured. This row rests on symmetry with `PreToolUse`**, and symmetry is an assumption (R12) | stdout -- same basis | already labelled -- `stop_gate_emit_blocking "[EDM] a stop-event hookify rule matched:" "$_hookify_out"` at `:244` | **Changed from "none".** AC2 updates `:238`/`:244`, AC6 moves the untrusted half to stdout, and **AC9 verifies by assertion that this gate's stdout is not returned to the model on exit 2** rather than assuming it. The label mechanism stays -- it is the in-tree precedent -- but "not changed" was true of the file and false of the behaviour |
...
| Project state file | `SRD/{PRODUCT}/{PREFIX}__{DESC}/.edm-state.json`, read-modify-write under `with_state_lock` (`_rmw_state_body`, applied at `bin/edm-state:5200`) | Filesystem | The double-completion check runs inside the single lock acquisition, so a refused completion mutates nothing (`bin/edm-state:5055-5056`, `:5072-5073`). EDMDS-10 AC2 adds a **project-scoped** lock for `SessionStart` marker reconciliation -- named as new work, because every lock in `bin/edm-state` goes through `with_state_lock "$lockbase"` with `lockbase="${f%.json}"` (`:848-851`), which is per-initiative and cannot serialize against a `phase-start 6` on a different initiative. AC4 then covers all **four** marker-mutation sites (`:2883`, `:3056`, `:3655`) |
...
the derived count for this requirement is **26**, so a hand-read list was four times short. The
source of truth is now `./affected-assertions.sh EDMDS-07`, which prints the set with `file:line` at
...
carry explicit controls (EDMDS-14 AC4, EDMDS-05 AC4) and both controls must fail before amendment to
...
20. **EDMDS-12** (accessor), **EDMDS-15** (the extracted `UserPromptExpansion` body with its five
    matchers retained, per D54), **EDMDS-05** (derived `bin/` membership). Independent of
